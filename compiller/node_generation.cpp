...
void IdentifierNode::generate(AsmCode &code) {
    ...
    if (var->global)
            code.add(_PUSH, new AsmMemoryArg("dword ptr [var_" + var->name  +"]"));
    else
 
            code.add(_PUSH, new AsmIndirectArg(_EBP, var->offset));
...
void PointerNode::generateLvalue(AsmCode &code){
	SymbolType *type = name->getType();
	SymbolType *t = type->upType();
	if (dynamic_cast<ArraySymbol*>(t)){
		t = t->upType();
	}
	if (dynamic_cast<IdentifierNode*>(name)){
		dynamic_cast<IdentifierNode*>(name)->generateLvalue(code);
		code.add(_PUSH, new AsmIntArg("0"));

	}
	else
	if (dynamic_cast<PointerNode*>(name)){
		dynamic_cast<PointerNode*>(name)->generateLvalue(code);
		
		code.add(_PUSH, new AsmIntArg("0"));

		code.add(_POP, _EAX);

		code.add(_MOV, _EBX,to_string( t->byteSize()));
		code.add(_IMUL, _EAX, _EBX);

		code.add(_POP, _EBX);
		code.add(_ADD, _EAX, _EBX);

		code.add(_PUSH, new AsmIndirectArg(_EAX));
		//code.add(_POP, new AsmRegArg(_EAX));
		//code.add(_PUSH, new AsmIndirectArg(_EAX));
		...
		dynamic_cast<BinOpNode*>(name)->left->generateLvalue(code);
		dynamic_cast<BinOpNode*>(name)->right->generate(code);
	}
...
void PointerNode::generate(AsmCode &code){
	generateLvalue(code);
	code
		.add(_POP, _EAX)
	.add(_PUSH, new AsmIndirectArg(_EAX));
		

}
void IdentifierNode::generateLvaluePointer(AsmCode &code){
}
