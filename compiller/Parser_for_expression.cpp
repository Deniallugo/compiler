...
FunctionalNode* Parser :: ParseFuncCall(ExprNode *r){
    FunctionalNode* f = new FunctionalNode(r->token, r
                                           //, dynamic_cast<FuncSymbol*>(r->getType())
                                           );
    scan.Next();
    Token* t = scan.Get();
...
//    //    }
//
//    //        errorIf(op->Type != _OPERATION && op->Value != ",", "Invalid expression. Operation expected",op);
//    if(priorityTable[op->Value] < priority)
//        return root;

    while(!scan.isEnd() && ((op->Type == _OPERATION && (priorityTable[op->Value] >= priority)
...
ExprNode* Parser :: ParseMember(ExprNode* left){
    SymbolType *type = nullptr;//left->getType();
...
VarSymbol * Parser :: ParseIdentifier(SymbolType *type, bool param){
    ...
    if(!isEq(token, _SEPARATION, "(")){
        ...
        if((symbolBuffer = symStack->tables.back()->find_symbol(name->Value)) != 0  ){
            ...
            FuncSymbol* func1 = createFunction(name->Value, type);
            errorIf(!CheckArgs(func), "Redefinition");
            result = new VarSymbol(name->Value, func1);
